...
def get_tweets(api, cacheDict, fname):
    ...
    if searchTerm in cacheDict:
        resultFound = cacheDict[searchTerm]
        return resultFound

    # otherwise get the data, add it the dictionary, and write it to a file
    else:
        resultFound = api.search(q = searchTerm)
        cacheDict[searchTerm] = resultFound
...
def setUpTweetTable(tweetList, conn, cur):
    ...
    cur.execute('CREATE TABLE Tweets(tweet_id INTEGER, author TXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)')
...
def getTimeAndText(cur):
    ...
    for tweet in cur.execute('SELECT time_posted, tweet_text from Tweets'):
        listTweet.append(str(tweet[0]) + ' - ' + tweet[1])
...
def getAuthorAndNumRetweets(cur):
    ...
    for tweet in cur.execute('SELECT author, retweets from Tweets'):
        if tweet[1] > 2:
            listTweet.append(tweet[0] + ' - ' + str(tweet[1]))
